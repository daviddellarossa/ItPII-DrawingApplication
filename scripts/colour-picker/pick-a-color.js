 ...
 *
 * This library has been modified by David Della Rossa and customized to the needs of this project.
 * A new tab has been added to control the Alpha channel and two new events have been created to notify when
 * a colour or the alpha channel amount change.
 * All modified lines are commented with // Modified by David Della Rossa
...
;(function ($) {
    ...
    $.fn.pickAColor = function (options) {
      ...
      var settings = $.extend({
        ...
        showAlpha             : true, // Modified by David Della Rossa
        showHexInput          : true,
        allowBlank            : false,
      ...
      var markupAfterInput = function () {
        var $markup = $("<div>").addClass("input-group-btn"),
            $dropdownButton = $("<button type='button' title='Colour selector'>").addClass("btn btn-default color-dropdown dropdown-toggle current-color"),
            /*
            * Modified by David Della Rossa
        ...
        */
        ...
        if (useTabs) {
          ...
          if(settings.showAlpha){
            $tabContainer.append($("<span>").addClass("alpha-tab tab").
            append($("<a>").text("Alpha")));
          }
        ...
        // Modified by David Della Rossa
        if (settings.showAlpha){
          var alphaColorsActiveClass = settings.showBasicColors || settings.showSavedColors ? 'inactive-content' : 'active-content';
          var $alpha = $("<div>").addClass("alpha-content").addClass(alphaColorsActiveClass),
              $alphaList = $("<ul>").addClass("alpha-list");

          var $alphaChannelItem = $("<li>").addClass("alphaChannel-item"),
              $alphaChannelSpectrum = $("<span>").addClass("color-box spectrum-alphaChannel");
          if (isIELT10) {
            $.each([0,1], function (i) {
              $alphaChannelSpectrum.append($("<span>").addClass("alphaChannel-spectrum-" + i +
                  " ie-spectrum"));
            });
          }
          var $alphaChannelHighlightBand = $("<span>").addClass("highlight-band");
          $.each([0,1,2], function () {
            $alphaChannelHighlightBand.append($("<span>").addClass("highlight-band-stripe"));
          });
          var $alphaChannelContent = $("<span>").addClass("alphaChannel-text").
          text("Alpha: ").append($("<span>").addClass("alphaChannel-value").text("255"));
          $alphaList.append($alphaChannelItem.append($alphaChannelContent).append($alphaChannelSpectrum.
          append($alphaChannelHighlightBand)));
          $dropdownContainer.append($alpha.append($alphaList));
        }
      ...
      var methods = {
        ...
        // Modified by David Della Rossa
        // calculates the amount of alpha based on the slider position
        getAlphaValue: function(){
          var $thisEl = $(this),
              hbWidth = $thisEl.outerWidth(),
              halfHBWidth = hbWidth / 2,
              position = parseInt($thisEl.css("left"),10),
              $alphaContainer = $thisEl.parents(".alpha-list"),
              spectrumWidth = parseInt($alphaContainer.find(".color-box").first().outerWidth(),10) - hbWidth - 2;
          return (Math.floor(position/spectrumWidth * 255));
        },
      ...
      return this.each(function (index) {
        ...
        if (settings.showAdvanced) {
          ...
          // Modified by David Della Rossa
          myElements.alphaHighlightBands = myElements.thisWrapper.find(".alpha-list").find(".highlight-band");
        }
        ...
        // Modified by David Della Rossa
        if(settings.showAlpha){
          $(myElements.alphaHighlightBands).on(dragEvent, function(event) {
            alphaStatus = methods.getAlphaValue.apply(this);
            $(".alphaChannel-value")[0].innerText= alphaStatus;
            $(myElements.thisEl).trigger("alphaChanged", [alphaStatus]);
          });
        }
